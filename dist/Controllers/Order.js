...
class Order extends Controller_1.default {
    constructor(client, db) {
        super(client, db);
        this.allowedFields = ['user', 'guild', 'order', 'userNotes', 'orderUserMentions', 'status', 'orderExpiry', 'chefNote', 'prepareExpiry', 'prepareDuration', 'deliveryAssignedTo', 'claimedBy', 'priority', 'sentAt', 'orderChannel', 'images'];
    }
    sendOrder(data) {
        return new Promise(async (resolve, reject) => {
            try {
                const orderID = this.generateOrderID(6);
                const query = await this.db.collection('orders').insertOne({ _id: orderID, ...data });
                ...
                this.client.controllers.discord.createMessage(this.client.clientOptions.discord.channels.orderLog, ':pizza: | ' + (data.priority ? ':exclamation: Priority Order :exclamation:\n️' : '') + ' (**`' + data.orderID + '`**) Order from: ' +
                    ...
                    '*Order was sent <t:' + (0, dateUtil_1.default)().unix() + ':R>*').catch(() => null);
    ...
    cancelOrder(orderID, user) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (!order)
                    return reject({ statusCode: 404, code: 'order_not_found', message: 'Order not found' });
                await this.client.controllers.backlog.addToLog({
                    ...
                    type: types_1.BacklogTypes.Cancel,
                    addedAt: Date.now()
                });
    ...
    getOrders(filter, fields = []) {
        return new Promise(async (resolve, reject) => {
            try {
                const query = await this.db.collection('orders').aggregate([
                    { $match: { ...filter } },
                    { $sort: { sentAt: 1, priority: 1 } },
                    { $project: (0, formatAggregate_1.default)([...this.allowedFields, ...fields], false, { _id: false, orderID: '$_id' }) }
                ]).toArray();
                resolve(query);
            }
            catch (e) {
    ...
    searchImages(query) {
        return new Promise(async (resolve, reject) => {
            try {
                const request = await axios_1.default.get('https://api.unsplash.com/search/photos?query=' + encodeURIComponent(query), {
                    headers: {
    ...
    claimOrder(orderID, user) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (order.user.id === user.id && !perms.hasPermission(types_1.Permissions.SUPER_ADMIN))
                    return reject({ statusCode: 400, code: 'order_unavailable', message: 'You cannot claim your own order' });
                if (order.status !== types_1.OrderStatus.Unclaimed || order.claimedBy && order.claimedBy.expiry <= Date.now()) {
                    if (order.status === types_1.OrderStatus.Claimed && order.claimedBy && order.claimedBy.user === user.id)
                        return resolve(order);
                    else
                        return reject({ statusCode: 400, code: 'order_unavailable', message: 'Order has already been claimed by another chef' });
                }
                const hasClaimedOrder = await this.getOrder({ status: types_1.OrderStatus.Claimed, 'claimedBy.user': user.id });
                if (hasClaimedOrder)
                    return reject({ statusCode: 400, code: 'active_order', message: 'You already have a claimed order' });
                const updatedOrder = await this.updateOrder(orderID, { claimedBy: { user: user.id, expiry: (0, dateUtil_1.default)().add(4, 'minutes').valueOf() }, status: types_1.OrderStatus.Claimed });
    ...
    unclaimOrder(orderID, user) {
        return new Promise(async (resolve, reject) => {
            try {
                const order = await this.getOrder({ _id: orderID, 'claimedBy.user': user.id, status: types_1.OrderStatus.Claimed }, ['claimedBy']);
                if (!order)
                    return reject({ statusCode: 404, code: 'order_not_found', message: 'Order not found' });
                const updatedOrder = await this.updateOrder(orderID, { claimedBy: null, status: types_1.OrderStatus.Unclaimed });
                resolve(updatedOrder);
            }
            catch (e) {
                reject(e);
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (!order)
                    return reject({ statusCode: 404, code: 'order_not_found', message: 'Order not found' });
                const perms = new Permissions_1.default(user?.permissions || 0);
                if (user && order.user.id === user.id && !perms.hasPermission(types_1.Permissions.SUPER_ADMIN))
                    return reject({ statusCode: 400, code: 'order_unavailable', message: 'You cannot delete your own order' });
                if (user && order.status !== types_1.OrderStatus.Claimed || !order.claimedBy) {
                ...
                await this.client.controllers.backlog.addToLog({
                    ...
                    orderedBy: order.user.id,
                    reason,
                    type: types_1.BacklogTypes.Delete,
                    addedAt: Date.now()
                });
                if (user) {
    ...
    prepareOrder(orderID, user, imageURLs, chefNote) {
        return new Promise(async (resolve, reject) => {
            try {
                const order = await this.getOrder(orderID, ['claimedBy']);
                if (!order)
                ...
                const perms = new Permissions_1.default(user.permissions || 0);
                if (order.user.id === user.id && !perms.hasPermission(types_1.Permissions.SUPER_ADMIN))
                    return reject({ statusCode: 400, code: 'order_unavailable', message: 'You cannot prepare your own order' });
                if (order.status !== types_1.OrderStatus.Claimed || !order.claimedBy) {
                    await this.claimOrder(orderID, user ? user : 'bot');
                }
                if (!imageURLs)
                    return reject({ statusCode: 400, code: 'invalid_body', message: 'You must provide at least one image URL' });
                if (!Array.isArray(imageURLs))
                    return reject({ statusCode: 400, code: 'invalid_body', message: 'Images must be an array' });
                if (!imageURLs.length)
                    return reject({ statusCode: 400, code: 'invalid_body', message: 'You must provide at least one image URL' });
                if (imageURLs.length > 3)
                    return reject({ statusCode: 400, code: 'invalid_body', message: 'A maximum of 3 images are allowed' });
                if (chefNote && chefNote.trim().length > 500)
                    return reject({ statusCode: 400, code: 'invalid_body', message: 'Chef note character limit has been exceeded' });
                const images = await this.validateImages(imageURLs);
                if (!images.valid.length)
                    return reject({ statusCode: 400, code: 'invalid_images', message: 'None of the images provided were valid' });
                if (images.valid.reduce((a, b) => a + b).length >= 1500)
                    return reject({ statusCode: 400, code: 'invalid_images', message: 'Images must have a shorter URL' });
                if (user) {
                    await this.addOrdersToUser(user);
                }
                await this.updateOrder(orderID, { chefNote, images: images.valid, prepareExpiry: Date.now() + (order.prepareDuration || 180000), status: types_1.OrderStatus.Preparing });
    ...
    deliverOrder(orderID, user, report = false, reportInfo = { reason: null }) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const perms = new Permissions_1.default(user?.permissions || 0);
                if (user && order.user.id === user.id && !perms.hasPermission(types_1.Permissions.SUPER_ADMIN))
                    return reject({ statusCode: 400, code: 'order_unavailable', message: 'You cannot deliver your own order' });
                if (order.status !== types_1.OrderStatus.PendingDelivery)
                    return reject({ statusCode: 400, code: 'order_unavailable', message: 'Order is not ready to be delivered yet' });
                const deliveryMessage = await this.getDeliveryMessage(user ? user : null, order);
                if (!report && deliveryMessage.length >= 1650)
                    return reject({ statusCode: 400, code: 'delivery_message_length', message: 'Delivery message length is too long, please change your message' });
                await this.client.controllers.backlog.addToLog({
                    orderID: order.orderID,
                    ...
                    userOrder: order.order,
                    orderedBy: order.user.id,
                    type: report ? types_1.BacklogTypes.Reported : types_1.BacklogTypes.Delivered,
    ...
    reportOrder(orderID, reason) {
        return new Promise(async (resolve, reject) => {
            try {
                await this.db.collection('order_reports').insertOne({ _id: orderID, reason });
    ...
    getOrderReports() {
        return new Promise(async (resolve, reject) => {
            try {
                const query = await this.db.collection('order_reports').find().toArray();
                resolve(query.map((r) => r._id));
            }
            catch (e) {
                reject(e);
            }
    ...
    getOrderReport(orderID) {
        return new Promise(async (resolve, reject) => {
            try {
                const query = await this.db.collection('order_reports').findOne({ _id: orderID });
                resolve(query);
            }
            catch (e) {
                reject(e);
    ...
    deleteOrderReport(orderID) {
        return new Promise(async (resolve, reject) => {
            try {
                await this.db.collection('order_reports').deleteOne({ _id: orderID });
                resolve();
            }
            catch (e) {
                reject(e);
    ...
    deleteOrderReports(query) {
        return new Promise(async (resolve, reject) => {
            try {
                await this.db.collection('order_reports').deleteMany(query);
                resolve();
            }
            catch (e) {
                reject(e);
    ...
    async getDeliveryMessage(user, order) {
        const defaultMessage = 'Hi, {user.mention}! ' +
        ...
        return message
            ...
            .replaceAll('{order.images}', order.images ? order.images.map((i) => i).join(' ') : '')
    ...
    validateImages(urls) {
        return new Promise(async (resolve) => {
            ...
            for (const url of urls) {
                try {
                    const image = await axios_1.default.get(url, { timeout: 5 * 1000 });
